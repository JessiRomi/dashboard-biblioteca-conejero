  ...
  <section class="space-y-6">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <button
        ...
        :class="tab === 'disponibles' ? 'bg-neutral-100' : ''"
        @click="tab = 'disponibles'"
        :hidden="!props.accionesHabilitadas"
      >
        Disponibles
      </button>
      <button
        class="px-3 py-1.5 rounded border text-sm"
        :class="tab === 'prestados' ? 'bg-neutral-100' : ''"
        @click="tab = 'prestados'"
        :hidden="!props.accionesHabilitadas"
      >
        Prestados
      ...
      <button
        class="px-3 py-1.5 rounded border text-sm"
        :class="orden === 'titulo' ? 'bg-neutral-100' : ''"
        @click="orden = 'titulo'"
      >
        Ordenar por título
      </button>
      <button
        class="px-3 py-1.5 rounded border text-sm"
        :class="orden === 'autor' ? 'bg-neutral-100' : ''"
        @click="orden = 'autor'"
      >
        Por autor
      </button>
      <button
        class="px-3 py-1.5 rounded border text-sm"
        :class="orden === 'paginasDesc' ? 'bg-neutral-100' : ''"
        @click="orden = 'paginasDesc'"
      >
        Por páginas (↓)
      </button>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-4">
      <article
        v-for="l in visibles"
        :key="l.id"
        class="rounded-xl border p-4 bg-white flex items-start justify-between gap-3"
      >
        <div>
          <h3 class="font-medium">{{ l.titulo }}</h3>
          <p class="text-sm text-neutral-500">Autor: {{ l.autor }} · {{ l.paginas }} págs.</p>
          <span
            class="inline-flex items-center mt-1 text-xs px-2 py-0.5 rounded"
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            :class="[
              ...
              !props.accionesHabilitadas ? 'opacity-40 pointer-events-none' : 'hover:bg-neutral-50',
            ]"
            :hidden="!props.accionesHabilitadas"
            v-if="l.disponible"
            @click="prestar(l.id)"
